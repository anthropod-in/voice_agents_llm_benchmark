...
1. **tool_use_correct** (bool):
   ...
   - FALSE if the model made duplicate tool calls (same function + same arguments called multiple times)
   - FALSE otherwise
   ...
   - IMPORTANT: If any tool call is marked as "is_duplicate": true, tool_use_correct must be FALSE
     (duplicate tool calls indicate model misbehavior)
...
2. **instruction_following** (bool):
   ...
   - TRUE if assistant gives a generic deflection for questions it cannot answer (even if golden shows a more nuanced response)
   - TRUE if assistant correctly calls end_session (even if assistant_text is empty or minimal - silent finish is acceptable)
   - FALSE if assistant neither answers nor advances the workflow
   - FALSE if assistant completely ignores the user's question
   - Be lenient: a generic "I can only help with the conference" deflection is acceptable even if the golden shows a more specific response
   - Examples of TRUE:
     ...
     - Deflecting "How do I get to Salon 2?" with "I can only help with the conference" (generic but acceptable)
     - Calling end_session with empty assistant_text (silent finish is OK)
...
def format_turns_for_claude(
    ...
    for rec in records:
        ...
        if actual_calls:
            ...
            # Check for duplicate tool calls and warn explicitly
            duplicates = [c for c in actual_calls if c.get('is_duplicate')]
            if duplicates:
                lines.append(f"WARNING: {len(duplicates)} duplicate tool call(s) detected - model misbehavior")
        else:
